...
export class AchievementService {
  ...
  async getAllAchievements(userId: number) {
    ...
    const achievementsWithProgress = await Promise.all(achievements.map(async achievement => {
      ...
      if (achievement.daily_steps_required !== null) {
        let userDailySteps = await this.activity.countDaysWith5000Steps(userId);
        progress = userDailySteps / achievement.daily_steps_required * 100;
        if (progress >= 100) {
          progress = 100;
        }
      }
  ...
  async updateCompletedAchievements(userId: number) {
    ...
    for (const achievement of missingAchievements) {
      ...
      if (achievement.daily_steps_required !== null) {
        if (achievement.daily_steps_required <= await this.activity.countDaysWith5000Steps(userId)) {
          isCompleted = true;
        }
      }
      if (isCompleted) {
